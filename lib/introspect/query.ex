defmodule Introspection.Query do
  @moduledoc """
  This module ports most of the basic functionality from the
  makeIntrospectionQuery function used in graphile's PgIntrospectionPlugin.

  See:
  https://github.com/graphile/graphile-engine/blob/v4/packages/graphile-build-pg/src/plugins/introspectionQuery.js
  ...
  def make_introspection_query(
    ...
    %{
      pg_legacy_functions_only: pg_legacy_functions_only,
      pg_ignore_rbac: pg_ignore_rbac
    } = options
    ...
    with
      #{if !pg_ignore_rbac, do: "recursive", else: ""} accessible_roles as (
        select oid _oid, pg_roles.*
        from pg_roles
        where rolname = current_user
        #{if !pg_ignore_rbac, do: union_rbac, else: ""}
      ),
      ...
      procedure as (
        ...
        where
          ...
          #{if server_version_num >= 110_000 do
      "pro.prokind = 'f'"
    else
      "pro.proisagg = false and pro.proiswindow = false"
    end} and
          #{if pg_legacy_functions_only do
      """
      ...
      "indexes" as (
        select
          ...
          #{if server_version_num >= 90600 do
      """
      (
